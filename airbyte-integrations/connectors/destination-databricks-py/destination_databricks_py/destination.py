...
from airbyte_cdk.models import (
    AirbyteConnectionStatus, AirbyteMessage, AirbyteStateType, ConfiguredAirbyteCatalog, DestinationSyncMode, Status, Type
)
...
def get_client(config: Mapping[str, Any]) -> dbxio.DbxIOClient:
...
class DestinationDatabricks(Destination):
    def write(
        ...
        stream_tables: Dict[str, dbxio.Table] = {}
        ...
        def flush_streams(streams: List[str]):
    ...
    def check(self, logger: AirbyteLogger, config: Mapping[str, Any]) -> AirbyteConnectionStatus:
        ...
        try:
            ...
            dbxio.bulk_write_table(
                table=t,
                new_records=[{FIELD_DATA: '{"key": "value"}', FIELD_EMITTED_AT: int(time.time()), FIELD_AB_ID: str(uuid4())}],
                client=client,
                abs_name=abs_name,
                abs_container_name=abs_container_name,
                append=True,
